...
cd /usr/src
sudo wget https://nodejs.org/dist/v11.0.0/node-v11.0.0.tar.gz
sudo tar zxf node-v11.0.0.tar.gz
sudo rm -f ./node-v11.0.0.tar.gz
cd node-v11.0.0
sudo ./configure
sudo make
sudo make install
cd ~
sudo rm -rf /usr/src/node-v11.0.0
sudo mkdir /ias
cd /ias

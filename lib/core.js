...
const serverConfigDefinitionFile = api.path.join(
  impress.dir, 'schemas', 'config.impress.definition'
);

const applicationConfigDefinitionFile = api.path.join(
  impress.dir, 'schemas', 'config.application.definition'
);

api.metaschema.load(serverConfigDefinitionFile, (err, schema) => {
  if (err) {
    console.error(err);
    return;
  }

  api.serverConfigDefinition = api.metaschema.build(schema);
})

api.metaschema.load(applicationConfigDefinitionFile, (err, schema) => {
  if (err) {
    console.error(err);
    return;
  }

  api.applicationConfigDefinition = api.metaschema.build(schema);
});

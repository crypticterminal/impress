...
const mixin = (application) => {
  ...
  application.require = (
    ...
    if (!exports) {
      try {
        ...
        if (!code) impress.log.error(impress.CANT_READ_FILE + fileName);
  ...
  application.loadConfigFile = (file, callback) => {
    ...
    application.createScript(configFile, (err, exports) => {
      ...
      if (api.metaschema.categories.has(sectionName)) {
        const validationResult = api.metaschema.validate(sectionName, exports);
        if (process.isMaster) {
    ...
    if (!application.isInitialized && !application.isImpress) {
      ...
      if (config.routes) { // Prepare application routes
        ...
        for (let j = 0; j < routes.length; j++) {
          ...
          const rx = !route.escaping ? route.url : ('^' + route.url
            .replace(/(\/|\?|\.)/g, '\\$1')
            .replace(/\(\\\.\*\)/, '(.*)') + '$'
          );
